...
const hrefs = [...html.matchAll(/href="([^"]+)"/g)].map((match) => new URL(match[1], "https://bedrock-agent.moretes.com/"));
const metas = [...html.matchAll(/<meta\s+([^>]+)>/g)].map((match) => match[1]);
const links = [...html.matchAll(/<link\s+([^>]+)>/g)].map((match) => match[1]);

const hasRepoLink = hrefs.some((url) => url.protocol === "https:" && url.hostname === "github.com" && url.pathname === "/fernandofatech/bedrock-agent-starter");
...
const hasCanonical = links.some((attrs) => attrs.includes('rel="canonical"') && attrs.includes('href="https://bedrock-agent.moretes.com/"'));
...
const required = [
  ...
  [html.includes("Bedrock Agent Starter"), "index.html must include the project title"],
  ...
  [robotLines.has("Sitemap: https://bedrock-agent.moretes.com/sitemap.xml"), "robots.txt must point to sitemap"],
  [sitemapLocs.some((url) => url.href === "https://bedrock-agent.moretes.com/"), "sitemap.xml must include canonical domain"],
