...
const hasRepoLink = links.some((link) => {
  const url = new URL(link, "https://bedrock-agent.moretes.com");
  return url.protocol === "https:" && url.hostname === "github.com" && url.pathname === "/fernandofatech/bedrock-agent-starter";
});
...
const required = [
  ...
  [html.includes('rel="canonical" href="https://bedrock-agent.moretes.com/"'), "index.html must include canonical domain"],
  ...
  [robots.includes("Sitemap: https://bedrock-agent.moretes.com/sitemap.xml"), "robots.txt must point to sitemap"],
  [sitemap.includes("https://bedrock-agent.moretes.com/"), "sitemap.xml must include canonical domain"],
